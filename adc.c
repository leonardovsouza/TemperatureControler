...
/* Method description: Return the value of the AD     */
/*                     converter, converted to        */
/*                     temperature in Celsius degrees.*/
/* Input params:  n/a                                 */
/* Output params: a float number with the temperature */
/* ************************************************** */
float adc_getTemperature(void)
{
    ...
    /* Checking if the conversion was done */
    if(adc_isAdcDone())
    {
        /* integer part */
        float fTemperature = adc_getConvertionValue();
        float fPosition = (fTemperature*255)/65535;
        int iPosition = (int)fPosition;

        /* Interpolation */
        float fFloatingNumber = (tabela_temp[iPosition+1] - tabela_temp[iPosition])*(fPosition-iPosition);
        return (float)tabela_temp[iPosition] + fFloatingNumber;
    }
    else
    {
        adc_getTemperature();
